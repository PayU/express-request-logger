...
function getRequestAudit(req, options) {
    ...
    let bodyStr = _.isEmpty(requestBody) ? NA: JSON.stringify(requestBody);
    if (options.request.maxBodyLength && bodyStr.length > options.request.maxBodyLength) {
        bodyStr = bodyStr.substr(0, options.request.maxBodyLength) + '...';
    }
...
function getResponseAudit(req, res, options) {
    ...
    let bodyStr = _.isEmpty(responseBody) ? NA: JSON.stringify(responseBody);
    if (options.response.maxBodyLength && bodyStr.length > options.response.maxBodyLength) {
        bodyStr = bodyStr.substr(0, options.response.maxBodyLength) + '...';
    }
