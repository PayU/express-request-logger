...
function getRequestAudit(req, options){
    var headers = _.cloneDeep(_.get(req, 'headers'));
    ...
    var requestBody =  _.cloneDeep(_.get(req, 'body'));
...
{
    if(body && fieldsToRemove.indexOf(ALL_BODY) == -1)
    {
        try {
...
function getResponseAudit(req, res, options){
    var headers = _.cloneDeep(_.get(res, 'headers'));
