...
function setBodyLengthFields(options) {
    ...
    if (!isValid(options.request.maxBodyLength)) {
        delete options.request.maxBodyLength;
    }
    if (!isValid(options.response.maxBodyLength)) {
        delete options.response.maxBodyLength;
    }
}